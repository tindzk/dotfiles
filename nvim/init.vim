...
" {{{ Basic syntax completion
	function! InsertClosing(closing)
		...
		if line[colmIdx] != a:closing
			if colmIdx == 0
				call setline('.', a:closing.line)
			else
				call setline('.', line[0:colmIdx-1].a:closing.line[colmIdx:])
			endif
		endif

		call cursor(line('.'), colm + 1)
		return ""
	endfunction

	function! CountUnbalanced(str, offset, end, open, close)
		let count = 0
		let index = a:offset
		while index < a:end
			if a:str[index] == a:close && count > 0
				let count -= 1
			elseif a:str[index] == a:open
				let count += 1
			endif
			let index += 1
		endwhile
	...
	function! InsertPair(opening, closing)
		...
		if a:opening == a:closing && line[colmIdx] == a:closing
			call cursor(line('.'), colm + 1)
			return ""
		endif

		let after = colmIdx
		while after < len(line)
			if line[after] !~ '\a' && line[after] != '.'
				break
			endif
			let after += 1
		endwhile

		let unbalanced = CountUnbalanced(line, 0, len(line), a:opening, a:closing)

		if colmIdx == 0
			if unbalanced == 0
				call setline('.', a:opening.line[:after-1].a:closing.line[after:])
			else
				call setline('.', a:closing.line)
			endif
		else
			if unbalanced == 0
				call setline('.', line[:colmIdx-1].a:opening.line[colmIdx:after-1].a:closing.line[after:])
			else
				call setline('.', line[:colmIdx-1].a:closing.line[colmIdx:])
			endif
		endif
	...
	function! SurroundMath(char)
		let line = getline('.')
		let colm = col('.')
		let colmIdx = colm - 1

		let unbalancedLeft  = CountUnbalanced(line, 0, colmIdx, a:char, a:char)
		let unbalancedRight = CountUnbalanced(line, colmIdx, len(line), a:char, a:char)

		" Check if cursor is already within a balanced math expression
		if unbalancedLeft > 0 && unbalancedLeft == unbalancedRight
			return ""
		endif
		...
		while before > 0
			let before -= 1
			if line[before] == a:char
				return ""
			elseif line[before] !~ '\a' && line[before] != '{' && line[before] != '\'
				break
			endif
		endwhile
		...
		let after = colmIdx + 1
		while after < len(line)
			if line[after] == a:char
				return ""
			elseif line[after] !~ '\a' && line[before] != '}'
				let after -= 1
				break
			endif
			let after += 1
		...
		if before == 0
			if after == 0
				call setline('.', a:char.a:char.line)
			else
				call setline('.', a:char.line[before:after].a:char.line[after+1:])
			endif
		else
			call setline('.', line[:before].a:char.line[before+1:after].a:char.line[after+1:])
		endif
	...
	inoremap { <C-R>=InsertPair('{', '}')<CR>
	inoremap } <C-R>=InsertClosing('}')<CR>
	autocmd FileType html,vim inoremap <buffer> < <C-R>=InsertPair('<', '>')<CR>
	autocmd FileType html,vim inoremap <buffer> > <C-R>=InsertClosing('>')<CR>

	autocmd FileType markdown inoremap <buffer> { <C-R>=InsertPair('{', '}')<CR><C-R>=SurroundMath('$')<CR>
	autocmd FileType markdown inoremap <buffer> $ <C-R>=SurroundMath('$')<CR>
	autocmd FileType markdown inoremap <buffer> * <C-R>=InsertPair('*', '*')<CR>
" }}}
